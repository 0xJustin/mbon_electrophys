...
try:
    ...
    print(
        "Failed to connect to neuprint. Check your token and make sure you are connected to the internet."
    )


def get_soma_location(bodyId):
    """
    Returns the x, y, and z coordinates of the soma for the given body ID.
    :param client: A neuprint client object.
    :param bodyId: A string representing the body ID to query.
    :return: A tuple of floats representing the x, y, and z coordinates of the soma.
    """
    soma = c.fetch_custom(
        f"MATCH (n:Neuron) WHERE n.bodyId={bodyId} RETURN n.somaLocation, n.somaRadius"
    )
    return soma["n.somaLocation"].iloc[0]["coordinates"], soma["n.somaRadius"].iloc[0]
...
def pull_synapses(pre_ids=None, post_id=612371421, n_iter=4, pre_or_post="pre"):
    ...
    for i in range(n_iter):
        ...
        if not pre_ids is None:
            presynaptic_neurons = pre_ids[
                i * n_neurons // n_iter : (i + 1) * n_neurons // n_iter
            ]
        ...
        for direction in directions:
            ...
            try:
                synapses = neuprint.fetch_synapse_connections(
                    presynaptic_criteria, postsynaptic_criteria, client=c
                )
            except Exception as e:
                print("ERROR: ", e)
                print(
                    "Failed to get synapses- probably a neuprint timeout, try increasing n_iter"
                )
...
def pre_syn_coord_dict(synapses, bodyids, factor=8 * (10 ** (-3))):
    ...
    for i in range(len(synapses)):
        ...
        y_scaled = factor * y
        z_scaled = factor * z
        coordinates[str(synapses.loc[i, "bodyId_pre"])].append(
...
def pull_skeleton(body_id, healdist=200, return_kdtree=False, skel_format="pandas"):
    ...
    """
    skel_mbon = neuprint.skeleton.fetch_skeleton(
        body_id, client=c, heal=healdist, format=skel_format
    )
...
def match_synapses_to_tree(synapses, skel_graph, relabel_dict):
    ...
    skel_coords = np.array(
        [
            [
                skel_graph.nodes[i]["x"],
                skel_graph.nodes[i]["y"],
                skel_graph.nodes[i]["z"],
            ]
            for i in skel_graph.nodes
        ]
    )
    synapses_coords = np.array(
        [
            [
                synapses.loc[i, "x_post"],
                synapses.loc[i, "y_post"],
                synapses.loc[i, "z_post"],
            ]
            if synapses.loc[i, "pre_or_post"] == "post"
            else [
                synapses.loc[i, "x_pre"],
                synapses.loc[i, "y_pre"],
                synapses.loc[i, "z_pre"],
            ]
